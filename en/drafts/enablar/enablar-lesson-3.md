...
Other tools within the iPySIGMA interface can make the graph easier to navigate and document. You can use the zoom controls to move closer to dense clusters or pull back to view the overall network structure. The center button returns the graph to the middle of the screen if you move too far away from it, while the reset button restores the original view and selections. Once the layout animation has settled the nodes into a readable arrangement, you can pause it to keep the graph stationary as you explore. You can also export a static image of the graph to use in presentations, reports, or comparisons between time periods. Finally, because some of the book titles and subject labels may appear in another language, Google Chrome’s translation feature can help translate the text while you navigate the network. 
...
Other subjects can be followed to examine periods of growth, decline, or changing relationships. Militaire wetenschappen (Military Sciences), for example, becomes increasingly connected to military topics toward the end of the nineteenth century and appears to peak during a particular period. This pattern could prompt questions about historical events, publishing activity, collecting priorities, or changes in subject description. A subject such as Sociale wetenschappen (Social Sciences) can also be explored through its connections to more specific areas such as sociology, political science, sexuality studies, and sekseverschillen (sex differences). Examining how these subjects cluster helps reveal when new areas of study become visible and how their relationships to broader fields change over time.

Individual books provide another way to compare the networks. Because this is a directed graph, the direction of a relationship matters. Subject headings often have a high outdegree because they connect outward to multiple book titles or to broader and narrower subject labels. A book node is more usefully examined through its indegree, which shows how many subject headings point toward and describe it. Following a recurring title such as the Bijbel (Bible) across the eight graphs could reveal changes in the number and types of subjects assigned to the same work. This makes it possible to examine not only whether a title remains present in the collection, but also whether its intellectual and cataloguing context changes over time.

The interface supports this type of focused comparison by allowing users to select a node from the menu and temporarily isolate its immediate connections. Nodes are also colour coded by subject grouping. In this example, the large number of subjects limits the usefulness of colour as a distinguishing feature, but it could be much more effective in a smaller network containing fewer than ten subject categories. Users can also search directly for a subject heading or book title, making it easier to locate the same concept or work in each chronological graph without relying on its visual prominence.
