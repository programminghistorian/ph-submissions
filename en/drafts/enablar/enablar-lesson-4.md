...
## Using Natural Language Processing on AI Licences

This lesson provides a reproducible workflow for interpreting AI tool licences through Natural Language Processing (NLP): the field of digital research involving interaction between computers and human language [^9]. NLP encompasses tasks such as text analysis, translation and parts-of-speech tagging, though we use it here to extract key terms and clauses in AI terms and conditions. As part of this, we present a dictionary of 'red flags' from licence conditions for you to experiment with. This lesson also provides a walkthrough for how to adapt and use this dictionary to increase your organisation's AI preparedness, alongside existing guidance [^10]. Though focused on AI licenced, we connect this concern to broader tool purchasing negotiated by libraries, collaboration and data sharing agreements.
...
While NLP often breaks text into individual word elements (tokens), spaCy also includes a parser to better understand the structure of agreements. We use this parser to reassemble tokens and extract full clauses, helpful for further - specialist - review. 
...
Note: this file upload will only last for the length of your session, and will not permanently keep your folder in the Colab environment. It will disappear if there is a runtime or service error, which forms one limitation of this approach compared to locally downloading Jupyter. 

If using your own licences, you can then add this code snippet to unzip your dragged folder and then follow the rest of our notebook code for NLP processing. 

    import zipfile

    with zipfile.ZipFile('/content/AI_Products_TOS.zip', 'r') as zip_ref:
