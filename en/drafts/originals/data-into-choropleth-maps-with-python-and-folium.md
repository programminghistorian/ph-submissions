...
You can fix this by deleting the legend that Folium creates (displaying log values) and replacing it with a new legend created by the [branca](https://python-visualization.github.io/branca/) library.

The following code:
1. Gets the tick values generated by Folium, saved to the `color_scale` attribute
1. Deletes the legend (called the `color_map`) created by Folium
1. Creates a new colormap
1. Assigns tick values by converting the log values to non-log equlivants
1. Adds a caption
1. Adds the legend / colormap to the map
