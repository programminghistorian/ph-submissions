...
In line 7, nanDECK is directed to set the font to size 14, bold and on a transparent background. Line 8 directs nanDECK to generate the text for `YEAR` in this font, and line 9 reduces the font size to 7.5 when applied to `FACT` text (generated in line 10). 
...
{% include figure.html filename="or-en-designing-a-timeline-tabletop-simulator-7.png" alt="Two pages of cards in a grid format. The left hand page of cards feature years printed in bold and the right hand side of cards are the same but without the year printed." caption="Figure 7. The simplest way for nanDECK to generate a card side without the date is to duplicate the information and remove the 'year' fied from one half." %}
