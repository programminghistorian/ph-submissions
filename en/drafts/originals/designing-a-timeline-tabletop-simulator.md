...
In this paper, we will be exploring how one can make their own versions of a tabletop game called _Timeline_.

## What is _Timeline_?

_Timeline_ is a card game designed by Frédéric Henry that was first published in 2012 and is still available for purchase through the global game publisher, Asmodee. The game of Timeline can be played with 2 to 8 players. As it is a very simple game to learn that can be played with a group of people, _Timeline_ is frequently described as a party game as well as an educational game. The game is published in several languages and there are a number of different themed versions of the game that have been published over the last ten years including, Timeline: Inventions, Timeline: Music and Cinema, Timeline: American History, and Timeline Star Wars.
...
The interface for nanDECK has been designed for function rather than fashion.

{% include figure.html filename="or-en-designing-a-timeline-tabletop-simulator-1.png" alt="Visual description of figure image" caption="Caption text to display" %}
...
{% include figure.html filename="or-en-designing-a-timeline-tabletop-simulator-2.png" alt="Visual description of figure image" caption="Caption text to display" %}
...
LINK is used to connect nanDECK to external data that has been formatted either as a text file with comma-separated values (CSV format) or in the form of an Excel spreadsheet (with xls, xlsx, or extensions). It is also possible to LINK to a Google Sheets spreadsheet and the additional steps necessary are outlined within the nanDECK Manual

nanDECK assumes that the first row of the linked data contains the names of the fields being imported.
...
{% include figure.html filename="or-en-designing-a-timeline-tabletop-simulator-3.png" alt="Visual description of figure image" caption="Caption text to display" %}
...
{% include figure.html filename="or-en-designing-a-timeline-tabletop-simulator-4.png" alt="Visual description of figure image" caption="Caption text to display" %}

### Line 6: IMAGE

While nanDECK includes a basic Visual Editor that allows for basic shapes to be drawn and added to cards, for the purposes of this deck, we want to add .jpg images from an external source that have been collected and copied from various digital history collections online and collected on a local file directory.
...
If we forget or are unsure which number nanDECK is associated g with which part of the syntax, you can mouse over the number and nanDECK will add bold to the relevant parameter.

{% include figure.html filename="or-en-designing-a-timeline-tabletop-simulator-5.png" alt="Visual description of figure image" caption="Caption text to display" %}
...
{% include figure.html filename="or-en-designing-a-timeline-tabletop-simulator-6.png" alt="Visual description of figure image" caption="Caption text to display" %}
...
```
7. FONT=Transport New Heavy,14,BT,#000000
8. TEXT="1-{(YEAR)}",\[YEAR],25%,60%,52%,9%
9. FONT=Transport New Light,7.5,,#000000
```
...
{% include figure.html filename="or-en-designing-a-timeline-tabletop-simulator-7.png" alt="Visual description of figure image" caption="Caption text to display" %}
...
nanDECK does the necessary calculations to generate a PDF that could be aligned together to create the final unique cards.

{% include figure.html filename="or-en-designing-a-timeline-tabletop-simulator-8.png" alt="Visual description of figure image" caption="Caption text to display" %}

## Printing Cards with nanDECK

nanDECK can print your deck in a multitude of ways. Each card generated by the program can be saved as a separate graphical image or, as described above, the deck can be printed to a PDF for printing, cutting, and gluing together. There is also the option to have your cards printed professionally. The creator of nanDECK has worked with _Game Crafter,_ an American print-on-demand company that specializes in producing cards and other tabletop game components, to allow for the easy upload of nanDECK files into their site for printing of single sets and limited runs.  

At this point it is worth noting that, in general, commercial publishers will not print any images that they believe are under somebody else’s copyright even if the game is intended for personal and educational use and not for commercial sale. Even though my local history version of Timeline was primarily drawn from public domain sources or from collections that had clear licenses that allowed for re-use, many of the images that I wanted to use in my cards fell under copyright. This means that there is a good chance that a commercial publisher might not want to publish even a single copy of my game.  

But there are other ways to bring people together to play our new game.  

## Printing Digital Cards for Tabletop Simulators

nanDECK also provides tidy integration with another platform called Tabletop Simulator.  

Tabletop Simulator (https://www.tabletopsimulator.com/) is available on the Steam platform (https://store.steampowered.com/app/286160/Tabletop_Simulator/) currently for $19.99 USD. While Tabletop Simulator is not the only digital platform that allows people to gather in front of a shared screen instead of a shared table to play cards and board games, it is a well-known title used by game designers to allow for game testing with players who may be physically dispersed around the world.  

The process to export nanDECK cards into TableTop Simulator (TTS) does require creating two separate decks - one of only the front of the cards and one of only the back of the cards. But once that is done, it is very simple to generate a single image of the cards for TTS to ingest and parse using nanDECK’s DISPLAY command.

```
LINK = Windsor-Timeline_for_Tabletop_Front.xlsx
…
rectangle=70,0,0,100%,100%,#000000
DISPLAY = "Windsor_Timeline_Front.png"
```

To be honest, I personally do not have a strong use case for making my local history mod of _Timeline_ available for play in _Tabletop Simulator_, other than the appreciation of how declarations of rules and systems can generate cards which themselves can be placed in a simulation of how these cards might operate in a physical system that includes the simulation of gravity.

{% include figure.html filename="or-en-designing-a-timeline-tabletop-simulator-9.png" alt="Visual description of figure image" caption="Caption text to display" %}

## Conclusion

I would like to end this paper with another tenet from Eric Zimmerman’s "Manifesto for a Ludic Century":
