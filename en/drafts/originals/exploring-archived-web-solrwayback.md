...
All input boxes contain an example text of kb.dk. In all steps below, this needs to be changed to a domain that is available in your small collection. Most of the time the lesson will use nasa.gov as the example here. The wordcloud tool generates domain wide wordclouds. These wordclouds can provide an overview of what is the most used words across a specific domain. To see an example of how this looks and how this could help you gain an overview of websites for people working in the american congress enter `house.gov` in the field to the left and press `Create wordcloud`. This compiles a wordcloud of the most used terms across the house.gov domain in you collection. This could be used to explore if words such as immigration where present many places across the house.gov domain.
...
Through SolrWayback it becomes possible to perform complex searches in material from web archives, that are otherwise often only available if you know the precise URL you are looking for. By navigating sources from the archived web in SolrWayback it becomes easier to work from a problem driven approach where research questions drive the exploration.  This lesson has taught you how to perform complex queries for material and navigate the archived web, which is quite often a temporal mess. 
