...
- At least one image downloaded to your local computer, not copyrighted by someone else. You can use the [Creative Commons Search Portal](https://search.creativecommons.org/) to find suitable images.
...
The [International Image Interoperability Framework](https://iiif.io/) describes itself as “a set of open standards for delivering high-quality, attributed digital objects online at scale.” In plain terms, IIIF if a standardized format for delivering digital objects, like images, videos, and even 3D models, from one place on the web to another. The framework allows cultural heritage institutions, project teams, and individuals to share high-resolution digital objets across the web quickly, easily, and in a way that speaks between and across different systems. 
...
IIIF manifests rely on [Uniform Resource Identifiers (URIs)](https://en.wikipedia.org/wiki/Uniform_Resource_Identifier) to identify and access IIIF-compliant images on the web and the display them using the IIIF Presentation API. There are two uniform identifiers that can be used to create manifests: the **info.json URI** and the **image URI**.
...
| Cons:<br>- No batch processing or editing (must upload one-by-one)<br>- No control over processing or the resulting image <br>-No deep zoom or image display specifications | Cons:<br>- No visual editor/interface<br>- Requires basic grasp of command line<br>- Requires knowledge of editing JSON files                                 | Cons:<br>- No visual editor/interface<br>- Requires **strong** grasp of command line<br>- Requires knowledge of editing JSON files                                         |
|                                                                                                                                                      |                                                                                                                                                               |                                                                                                                                                                        |

## Method 1: Minimal Level-0 on GitHub.com
### Overview

Method 1 employs GitHub Pages to host the simplest possible Level-0 manifest, without the need for the an image server/use of the Image API. This method requires only the image file and a barebones `info.json` file, and does not require any programming knowledge or installations.
...
The first step is to upload your image to a web server. For the sake of this lesson, we will be uploading an image to a repository on GitHub.com and serving through GitHub Pages, where we will also host the manifests later.
...
Method 2 employs the use of IIIF Workbench, a tool created by IIIF Technical Coordinator [Glen Robson](https://github.com/glenrobson) that allows you to upload an image from your local computer that will then be stored as converted to IIIF tiles and stored in your own version of the IIIF Workbench GitHub repository. 
...
Method uses libvips, an image-processing library, instead of IIIF Tiler to create Level 0 IIIF-compliant image tiles. It then uses GitHub to host those images and present them using the IIIF Presentation API through GitHub Pages.
...
**Note that for an untiled image (Method 1), your last `id` field should include the basic URL to your image. For tiled images (Methods 2 and 3), your last `id` field should include the Image URI API.**
...
- How to process images downloaded on your computer into IIIF-compatible tiles using the iiif-tiler command line tool
