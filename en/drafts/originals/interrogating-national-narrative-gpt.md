...
To interrogate the national narrative generated and disseminated by media coverage regarding Brexit, our input for this lesson will be [a dataset of 4036 web-published news articles](https://github.com/programminghistorian/ph-submissions/blob/gh-pages/assets/interrogating-national-narrative-gpt/articles.txt) derived from popular news sources in the UK such as BBC and *The Sun* written on the topic of "Brexit", extracted from the [UK Web Archive's curated "News and Media" collection on this subject](https://www.webarchive.org.uk/en/ukwa/collection/912). First, [the links to each article were gathered](INSERT SCRAPING LINK) so that each article could be extracted from its respective webpage in bulk via web scraping through a [Python script implementing the Beautiful Soup library](INSERT SCRAPING ARTICLES). The articles were then exported collectively into a singular large `.txt` file. The text of each article was then cleaned of any additional HTML tags or unintentionally captured information, such as footer menus, using regular expressions (regex).
