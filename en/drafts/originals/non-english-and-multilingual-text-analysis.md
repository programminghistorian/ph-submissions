---
title: "Introduction to Text Analysis for Non-English and Multilingual Texts: Using NLTK, spaCy, and Stanza to Process a Text in French and Russian Using Part-of-Speech Tagging, Language Detection, and Lemmatization"
...
Now, let's remove the newline characters to make the text easier to work with computationally. We don't need to perform any additional pre-processing steps for the purposes of this tutorial, since we will primarily be focusing here on our specific  methods of tokenization, language detection, POS tagging, and lemmatization. For a good introduction to preparing your text for multilingual analysis, please consult [this article.](https://modernlanguagesopen.org/articles/10.3828/mlo.v0i0.294) 
...
The entire text is now accessible as a list of sentences within the variable nltk_sent_tokenized. Let's print three sentences we'll be working with: one entirely in Russian, one entirely in French, and one that is in both languages. The language of the sentences will become important as we apply different methods to the sentences later in the lesson.
