...
You can download the data yourself using YouTube Data Tools and inputting the same video IDs, but please note the data will likely differ based on when you capture the data.
...
The R script for this lesson and the sample data are [available to download](https://github.com/programminghistorian/ph-submissions/tree/gh-pages/assets/text-mining-youtube-comments). The rest of this lesson will walk you through the steps needed to create an R script from scratch, writing out each step of that same code available for download. This lesson and the code can easily be adapted to alternative datasets downloaded through YouTube Data Tools.

### Installing R Environment and Libraries
...
In the next step, you will optimize the corpus to focus on the most meaningful words only. The following code removes words with fewer than four characters, as well as rare words (those that appear in less than 1% of documents, or that comprise less than .001% of the total corpus):
...
[Slapin and Proksch](https://onlinelibrary.wiley.com/doi/10.1111/j.1540-5907.2008.00338.x) (2008) first proposed the Wordfish model as an advancement over previous text-scaling methodologies.[^9] They use Wordfish to estimate the ideological positions of German political parties around the turn of the century.
...
The code in the this section will enable you to create word-level visualizations. 
...
The fact that this divide does not manifest in the visualization suggests that left-leaning and right-leaning video channels are receiving comments from people spread across the political spectrum. Instead, the most apparent dimension is a divide between comments that focus on the concrete event of the police killing of George Floyd, in comparison with comments that discuss the broader social context. Within that dichotomy, there is a noticeable tendency for descriptions of the event to use langauge associated with a left-leaning perspective, whereas descriptions of the context seem to evoke language associate with right-leaning perspectives.
...
If you used this lesson's data, you were able to tell from these visualizations that a broadly similar set of topics is discussed on comment threads for both left-leaning and right-leaning YouTube videos focused on police brutality and police funding in the United States since 2020. You also saw that you can interpret these visualizations to discover which words determined the scale created by the Wordfish model, and which videos contributed to each pole of that scale. 
...
[^1]: Rieder, Bernhard. _YouTube Data Tools_. Version 1.42. (Software). <https://ytdt.digitalmethods.net>.
