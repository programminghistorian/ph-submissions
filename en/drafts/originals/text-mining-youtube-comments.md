...
Now that the model is built, you can visualize its output. Wordfish models are well-suited to generating two distinct kinds of visualizations: a 'document-level' visualization and a 'word-level' visualization, both of which are scaled along horizontal and vertical axes. 
