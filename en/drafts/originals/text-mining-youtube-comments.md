...
YouTube is the most popular web-based video sharing and viewing platform in the world, with billions of users viewing and uploading videos each month. This lesson overviews how to gather and explore YouTube video comment data. After explaining how to access and query YouTube's application programming interface (API) with the user-friendly tool, YouTube Data Tools this lesson will then demonstrate how to text mine the comment data in R using Wordfish, an unsupervised text scaling algorithm for analyzing politically polarized dimensions salient in corpor.
...
For this lesson, the comment data was gathered by searching YouTube for "black lives matter george floyd". Choosing multiple videos is often the best approach for the exploratory stages of research, especially because the YouTube API does not always return data for every video searched, even if comment data for that video exists. 

Since this lesson focuses on the Wordfish algorithm, the selected videos should be drawn from polar opposite creators (channels) with a substantial number of comments to model (e.g., at least 5000 comments). The videos for this lesson were selected from politically polarized news sources (ranked by allsides.com), including the "left-leaning" sources of New York Times, Vox, and NBC News and the "right-leaning" Daily Mail, Fox News, and the Daily Wire.
...
## Import the Dataset
Once you've downloaded your dataset, to read in a .csv of already downloaded comments and metadata, use the following code first. You may also be able to use a comment dataset not downloaded using this tool, however, you'll first need to ensure it is formatted the same way data downloaded with this tool would be (e.g. possibly reordering and re-naming columns).
...
Because Wordfish is designed to measure difference along a one-dimensional scale, for purposes of this lesson, the YouTube video channels were selected from YouTube channels with political leanings that are generally classified as belonging to one of two groups, "right" and "left". 
...
This visualization shows all of the words found in the corpus comments. You can read the outlier words on the far right and left of the visualization pretty easily. But, given the large number of words displayed, the dense cluster of words in the middle of the visualization can be difficult to read and interpret.
...
You will notice in the above visualization that there is a clear horizontal separation between words. Most of the comments are located in the middle of the horizontal dimension, with a few outliers at each end. There are many ways to interpret the substantive meaning of this scaling.  Below we offer two ways to interpret the results.
