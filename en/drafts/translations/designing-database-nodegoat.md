...
- [walk through the first steps in building a database using the online platform](#database_id) 
...
Thanks to this new table, we can now cross-reference more information and go into more depth in our analysis by querying the relationships between various attributes of the works, the publishing houses, and the authors. We could, for example, formulate and text a hypothesis about whether Russian authors tend to publish more frequently in French, or ask whether certain publishing houses show a preference for authors of a particular age or nationality.
...
The aim of this lesson is to help researchers in the humanities conceptualize their research in terms of data by introducing them to the basics of modeling and building a database, which is often challenging for newcomers. It offers an entry point to using the online platform nodegoat, which is particularly well suited to the needs of those new to digital data management. 
