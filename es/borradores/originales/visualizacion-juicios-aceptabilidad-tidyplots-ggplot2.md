...
- `instruction`: instrucción bilingüe vs. clases de ELE
...
El resultado es el gráfico en Figura 1 que revela una tendencia interesante con respecto a la interacción entre la L1 y el tiempo verbal. Los hablantes nativos muestran una preferencia más clara por el PPS sobre el PPC (media ≈  5.5 para el PPS, y ≈ 4 para el PPC), mientras que los juicios del PPC y el PPS por los aprendientes muestran una distribución diferente (media ≈  4 para el PPS y media ≈  5 para el PPC).


{% include figure.html filename="es-or-visualizacion-juicios-aceptabilidad-tidyplots-ggplot2-01.png" alt="Gráfico de violín que muestra la distribución de los juicios de aceptabilidad (1–7) según la L1 (eslovaco vs. español) y el tiempo verbal (PPC vs. PPS). Los hablantes de español muestran un contraste más marcado entre tiempos verbales que los de eslovaco." caption="Figura 1. Distribución de los juicios de aceptabilidad según la L1 (eslovaco vs. español) y el tiempo verbal (PPC vs. PPS)." %} 
...
{% include figure.html filename="es-or-visualizacion-juicios-aceptabilidad-tidyplots-ggplot2-02.png" alt="Gráfico de violines que compara los juicios de aceptabilidad (escala 1-7) de hablantes de eslovaco y español para los tiempos verbales PPC y PPS, según tres tipos de adverbio (anterioridad inmediata, anterioridad lejana, momento no terminado). Los hablantes de español muestran un patrón claro: prefieren PPC con momento no terminado, y PPS con anterioridad lejana, mientras que anterioridad inmediata combina tanto con PPS (media: 5.8) como con PPC (media: 3.6). Los hablantes de eslovaco muestran juicios más dispersos para ambos tiempos verbales en todos los tipos de adverbio." caption="Figura 2. Juicios de aceptabilidad por tiempo verbal y tipo de adverbio temporal según grupo de L1. Los violines muestran la distribución de las puntuaciones; los puntos indican la media." %} 
...
{% include figure.html filename="es-or-visualizacion-juicios-aceptabilidad-tidyplots-ggplot2-03.png" alt="Gráfico de violín comparando juicios de aceptabilidad (1–7) por tiempo verbal y tipo de adverbio, en aprendientes bilingües vs. ELE. La anterioridad inmediata y el momento no terminado muestran juicios más altos con PPC que con PPS; la anterioridad lejana muestra el patrón inverso. Los patrones son similares entre ambos tipos de instrucción." caption="Figura 3. Juicios de aceptabilidad por tiempo verbal y tipo de adverbio temporal según el tipo de instrucción (bilingüe vs. ELE). Los violines muestran la distribución de las puntuaciones; los puntos indican la media." %} 

Este gráfico responde directamente a una pregunta de investigación interesante: ¿difieren los juicios de los aprendientes con instrucción bilingüe y los que recibieron clases de ELE? Como se puede observar en el gráfico, parece que los juicios de ambos grupos son comparables y no cambian mucho con el tipo de instrucción. El gráfico indica que ambos grupos prefieren los adverbios de anterioridad inmediata con el PPC (media ≈  5.6 grupo bilingüe, media ≈ 5.4 grupo ELE) y los adverbios de anterioridad lejana con el PPS (medias ≈ 4.4 grupo bilingüe, media ≈ 4.6 grupo ELE) y momento no terminado con el PPC (media ≈ 4.3 grupo bilingüe, media ≈  4.4 grupo ELE). Las similitudes en los juicios por los dos grupos se podrían atribuir al contexto geográfico (aunque uno de los grupos recibe clases de ELE y el otro tiene algunas de sus asignaturas impartidas en español, ambos estudian español en su país de origen).
...
{% include figure.html filename="es-or-visualizacion-juicios-aceptabilidad-tidyplots-ggplot2-04.png", alt="Gráfico de cajas comparando juicios de aceptabilidad (1–7) por tipo de adverbio y tiempo verbal (PPC vs. PPS), entre hablantes eslovacos y españoles. Los patrones de diferencia entre grupos varían según la combinación de tiempo verbal y tipo de adverbio, sugiriendo una interacción entre estos factores. Con PPC, los españoles muestran medianas más altas que los eslovacos en momento no terminado y más bajas en anterioridad lejana y anterioridad inmediata. Con PPS, los españoles muestran medianas más altas en anterioridad lejana y anterioridad inmediata y medias más bajas con momento no terminado." caption="Figura 4. Juicios de aceptabilidad por tipo de modificador adverbial, grupo y tiempo verbal. Los diagramas de caja muestran la mediana, el rango intercuartílico (caja) y los valores mínimo y máximo excluyendo valores atípicos (bigotes) para cada condición. Los puntos indican valores atípicos individuales." %} 
...
linechart01
```
{% include figure.html filename="es-or-visualizacion-juicios-aceptabilidad-tidyplots-ggplot2-05.png", alt="Dos paneles de diagramas de líneas que muestran los juicios de aceptabilidad medios de hablantes nativos y aprendientes de español para tres tipos de modificadores adverbiales (anterioridad inmediata, anterioridad lejana y momento no terminado), separados por tiempo verbal (PPC y PPS)." caption="Figura 5. Juicios de aceptabilidad por tipo de modificador adverbial, grupo y tiempo verbal." %} 
...
scatterplot01
```
{% include figure.html filename="es-or-visualizacion-juicios-aceptabilidad-tidyplots-ggplot2-07.png" alt="Diagrama de dispersión que muestra la relación entre la edad de adquisición del español (aoa, eje X) y los juicios medios de aceptabilidad (1–7, eje Y) para aprendientes con instrucción bilingüe (verde) y ELE (morado). Ambos grupos muestran una tendencia negativa leve, con mayor variabilidad en el grupo ELE." caption="Figura 7. Relación entre la edad de adquisición del español y los juicios medios de aceptabilidad por tipo de instrucción (bilingüe vs. ELE). Cada punto representa la media de juicios de un participante. " %} 

El gráfico de dispersión nos muestra que ambos grupos tienen sus juicios concentrados cerca del punto medio de la escala y los puntos de los dos grupos están entremezclados entre sí, sin mostrar una correlación con la edad de adquisición. 
...
ggplot(datos, aes(x = variable_x, y = variable_y, color = grupo)) +
  ...
  theme_classic()
```
Para aprender a visualizar en ggplot2 los gráficos que hemos visto en esta lección hasta ahora (gráficos de dispersión, diagramas de barras y diagramas de caja), te recomendamos la lección de Programming Historian *Visualizing Urban and Demographic data in R with ggplot2* (https://programminghistorian.org/en/lessons/urban-demographic-data-r-ggplot2). Si te gustaría aprender cómo organizar tus datos en el formato tidy y cómo como utilizar el paquete dplyr para manipular tus datos, te recomendamos la lección *Data Wrangling and management in R* (https://programminghistorian.org/en/lessons/data-wrangling-and-management-in-r). 
...
{% include figure.html filename="es-or-visualizacion-juicios-aceptabilidad-tidyplots-ggplot2-08.png" alt="Gráfico de dispersión con dos líneas de regresión para representar la tendencia lineal de cada grupo." caption="Figura 8. Gráfico de dispersión con juicios medios de aceptabilidad en función de la edad de adquisición del español según el tipo de instrucción y con las líneas de regresión para representar la tendencia lineal de cada grupo y con una línea horizontal punteada en el valor de la media global de todas las medias de aceptabilidad." %} 

La línea verde (instrucción bilingüe) indica que entre los participantes con la instrucción bilingüe, la edad más alta (18 años) se relaciona con juicios más altos (≈  4.5) y la edad más temprana (7 años) con juicios más bajos (≈  3.8), mientras que para los participantes con la instrucción ELE (línea morada), no se asocia con cambios en juicios medios (≈ 4.2 - 4.3 en todo el rango de edades). 
...
{% include figure.html filename="es-or-visualizacion-juicios-aceptabilidad-tidyplots-ggplot2-09.png" alt="Gráfico de dispersión que muestra la relación entre la edad de adquisición del español (AoA) y los juicios medios de aceptabilidad en hablantes eslovacos, diferenciando dos tipos de instrucción: bilingüe (círculos amarillos) y ELE (triángulos azules)." caption="Figura 9. Juicios medios de aceptabilidad según la edad de adquisición (AoA) y el tipo de instrucción en hablantes eslovacos." %} .
...
{% include figure.html filename="es-or-visualizacion-juicios-aceptabilidad-tidyplots-ggplot2-10.png" alt="Matriz de gráficos de violín con diagramas de caja que muestra los juicios de aceptabilidad (1–7) según el tipo de instrucción (Bilingüe vs. ELE), el tiempo verbal (PPC vs. PPS) y el tipo de modificador adverbial. El PPC con anterioridad inmediata muestra puntuaciones más altas en ambos grupos que anterioridad lejana y momento no terminado, mientras que el PPS obtiene las puntuaciones más altas con anterioridad lejana, seguida por el momento no terminado y anterioridad inmediata." caption="Figura 10. Distribución de los juicios de aceptabilidad por tipo de instrucción (Bilingüe vs. ELE), tiempo verbal (pretérito perfecto compuesto vs. pretérito perfecto simple) y tipo de modificador adverbial. Cada violín muestra la densidad de las respuestas y el diagrama de caja integrado indica la mediana y el rango intercuartílico. Los puntos representan valores atípicos." %} 
...
Para explorar simultáneamente las relaciones entre todas las variables continuas de los aprendientes, la matriz de dispersión (scatterplot matrix) es una herramienta muy eficiente. Se genera con la función `pairs()` del paquete base R. 
...
```
{% include figure.html filename="es-or-visualizacion-juicios-aceptabilidad-tidyplots-ggplot2-11.png" alt="Matriz de dispersión que muestra las relaciones entre el juicio medio de aceptabilidad, la edad de adquisición y los años de aprendizaje, diferenciando dos grupos mediante círculos rojos y cruces verdes." caption="Figura 11. Matriz de dispersión de las variables mean_rating, aoa y years_learning según grupo (○ = bilingüe 1; × = ELE)." %} 

Esta matriz de gráficos de dispersión muestra las asociaciones entre el juicio medio de aceptabilidad (**mean_rating**), la edad de adquisición (**aoa**) y los años de aprendizaje (**years_learning**) según el grupo (círculos rojos y cruces verdes). En general, no se observan correlaciones lineales fuertes entre las variables, aunque destaca una posible correlación negativa entre **aoa** y **years_learning**. Los dos grupos presentan una distribución solapada en la mayoría de los paneles, con diferencias leves en **mean_rating**. 
...
El tipo más sencillo de efecto aleatorio es el intercepto aleatorio: permite que cada participante (o ítem) tenga su propio punto de partida en la escala de respuesta. En notación de R con los paquetes `lme4` o `ordinal`, se especifica como `(1 | participant_id)`.
...
[^2]: Los esquemas de colores de tidyplots están disponibles en: https://jbengler.github.io/tidyplots/articles/Color-schemes.html
[^3]: Debido a que el modelo más complejo (modelo1.1) no mostró problemas de convergencia ni singularidad, también podrían reportarse los resultados de este modelo (los modelos 1.1. y 1.2. muestran que los mismos efectos e interacciones son estadísticamente significativas). La decisión de seleccionar el modelo con la estructura de los efectos aleatorios máxima, recomendada por Barr et. al. (2013) vs. mediante la selección del modelo recomendado por Matuschek et al (2017) depende de varios factores (el tamaño de la muestra, la complejidad del diseño experimental, el objetivo del análisis, etc.,) sobre los cuales se puede leer detalladamente en los dos artículos.  
