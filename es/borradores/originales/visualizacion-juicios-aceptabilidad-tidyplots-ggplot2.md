...
`ggplot2` es el sistema de visualización de referencia en R, basado en la [gramática de gráficos](https://perma.cc/26U2-6WBQ). Ofrece un control total sobre cada elemento del gráfico, lo que lo hace indispensable para publicaciones, para tipos de gráficos no disponibles en `tidyplots` (como la matriz de gráficos de dispersión) y para personalizaciones avanzadas. Si quieres aprender más sobre las ventajas de utilizar `ggplot2`, puedes consultar la sección [Ventajas de ggplot2](https://perma.cc/Y8T6-PCXV) de la lección [Visualizando datos urbanos y demográficos en R con ggplot2](https://perma.cc/3T7Y-CUAZ) de *Programming Historian en español*. 
...
Una explicación visual de la diferencia entre los interceptos y las pendientes aleatorias se puede consultar en [este enlace](http://mfviz.com/hierarchical-models/).
...
Podemos reportar e interpretar el output del modelo 1.2 de la siguiente manera: 
...
Para obtener medias marginales estimadas y comparaciones por pares con [corrección de Bonferroni](https://perma.cc/CC7K-EXQ7), utilizamos el paquete `emmeans`: 
...
Con respecto a las interacciones, tanto la anterioridad lejana (β = 3.68, z = 12.89, p < .001) como el momento no terminado (β = 2.22, z = 8.10, p < .001) hacen que el PPS sea juzgado de forma significativamente más favorable en comparación con el PPC, respecto a lo observado en la condición de referencia; de hecho, en el caso de la anterioridad lejana, el patrón se invierte por completo, y los aprendientes juzgan el PPS como más aceptable que el PPC.
