...
En esta lección es necesario utilizar la línea de comandos. Si no tienes experiencia previa en utilizarla consulta la lección [Introducción a la línea de comandos en Bash](introduccion-a-bash) de *Programming Historian*.
...
La computadora imprime las palabras clave, las palabras que ayudan a definir un tópico estadísticamente significativo, según la rutina. En la figura 9, el primer tópico que se imprime podría verse así (tus palabras clave podrían ser un poco diferentes):[^9]
...
¿Qué tópicos componen tus documentos? La respuesta está en el archivo `leccion_topicos_en_docs.txt`. Para mantenerte organizado, importa el archivo `leccion_topicos_en_docs.txt` a una hoja de cálculo (en Excel, Open Office, etc.). Tendrás una tabla con las columnas *número de documento, fuente, proporción del tópico*. Todas las columnas subsiguientes son proporciones de los tópicos, como en la figura 11.[^12]
...
## Notas de traducción
[^1]: En esta traducción se utiliza la expresión *topic modeling* en inglés porque en la literatura publicada sobre el tema en español es lo más común que se utilice la expresión inglesa. Por supuesto sería posible traducir *topic modeling* por modelaje de tópicos o algo parecido, pero hasta ahora no es la expresión habitual. Por otro lado, se ha optado por traducir todas las demás palabras relacionadas al método para estimular su uso en español, por ejemplo tópico para *topic* o modelo de tópicos para *topic model*.
