...
Software Heritage permet un accès unifié à des fonds issus d'une grande diversité de gisements. Outre les plateformes de développement et de distribution les plus communément utilisées, Software Heritage fournit aussi un accès aux :

* contenus publics de [forges désactivées comme Google Code](https://archive.softwareheritage.org/browse/search/?q=googlecode.com&visit_type=git&with_content=true&with_visit=true) ; 
* contenus des forges institutionnelles basées sur GitLab (par exemple [l'instance GitLab de l'IN2P3 du CNRS](https://gitlab.in2p3.fr/)) ayant fait la demande d'être versées dans l'archive ; 
* codes sources associés à des articles, grâce à ses partenariats avec des revues en sciences et technologies ; 
...
> « C'est un numéro ou une étiquette alphanumérique, opaque ou explicite, lisible par des machines et par des humains, permettant de désigner et de retrouver de manière univoque et pérenne un objet, un document, une personne, un lieu, un organisme, ou toute entité, dans le monde réel et sur internet[^8]. »

Les identifiants réduisent les ambiguïtés : il peut être en effet parfois difficile de savoir si tel auteur ayant publié dans une discipline donnée est l'homonyme d'un autre auteur, ayant publié dans un domaine proche ou s'il s'agit d'une seule personne, active dans plusieurs champs. On peut également citer le cas des institutions, dont le nom peut changer au fil du temps. Plusieurs institutions peuvent fusionner et former une nouvelle entité à décrire. Ces changements rendent difficiles l'identification des ressources produites par ces instances.
...
5. utilisateur avancé, vous possédez un _code de hachage_ permettant d'identifier un certain fichier, un répertoire, une version ou une révision ; 6. vous possédez un *SoftWare Hash Identifier* (SWHID).

#### Cas d'étude quand vous explorez Software Heritage 

Examinons chacun des cas présent, en allant du point de départ le plus imprécis vers le plus précis. Les options 1, 2, 4 et 6 permettent une interrogation facilitée de Software Heritage (Fig. 1).

{% include figure.html filename="entry-points2.png" caption="Figure 1 : Tous les chemins mènent à Software Heritage." %}
...
3. le code correspondant aux dépôts https://github.com/CatalaLang/catala et [rsync://ftp.gnu.org/gnu/gcc/gcc-4.9.3](rsync://ftp.gnu.org/gnu/gcc/gcc-4.9.3)_; 
...
{% include figure.html filename="exercice_tm.gif" caption="Figure 3 : Trouver et citer un extrait de code source précis dans l'archive Software Heritage." %}
...
{% include figure.html filename="creer_projet_gitlab.png" caption="Figure 6 : Créer un nouveau projet dans l'interface de GitLab.com." %}
...
{% include figure.html filename="exercice_leap.gif" caption="Figure 7 : Ajout du fichier `leap.sh` dans votre dépôt." %}
...
{% include figure.html filename="exercice_licence.gif" caption="Figure 8 : Ajout d'une licence." %}
...
{% include figure.html filename="exercice_save_now.gif" caption="Figure 9 : Archiver manuellement un dépôt.." %}
...
{% include figure.html filename="exercice_save_plugin.gif" caption="Figure 10 : Archiver un dépôt à l'aide du plug-in de navigateur." %}

## Pour aller plus loin : décrire un logiciel avec CodeMeta 

Fournir [un fichier README suffisamment détaillé](https://archive.softwareheritage.org/swh:1:cnt:0918fd32b5eddc85aaaeb35da83a749795e703bc;origin=https://github.com/wfinsinger/tapas;visit=swh:1:snp:fb13c70f7d5edc7515756d6f1c01a3f3b1a69e72;anchor=swh:1:rev:5f3d58423904da1b0bf368886ccf31d97956f7b3;path=/README.Rmd) constitue une étape importante pour expliquer les finalités d'un logiciel. Ajouter un fichier descriptif CodeMeta facilite l'identification d'un logiciel.
...
1. Les métadonnées intrinsèques : elles fournissent des informations essentielles pour assurer la préservation et l'utilisabilité à long terme des logiciels. Elles sont contenues dans un fichier texte à vocation descriptive, souvent intitulé `README` ou `DESCRIPTION`, situé à la racine du répertoire du code source du logiciel. C'est pourquoi, très souvent, les auteurs de logiciels sont les premiers et les principaux fournisseurs de métadonnées intrinsèques.

2. Les métadonnées extrinsèques peuvent fournir quant à elles des informations importantes sur le contexte et la provenance du logiciel : par exemple, sur quelle forge le logiciel a-t-il été développé ? Ces informations aident les utilisateurs à localiser le logiciel, à identifier la communauté qui l'utilise. Ces métadonnées renseignent aussi sur la relation entre le logiciel et d’autres produits de recherche : des publications, des jeux de données. Ces métadonnées ne sont pas incluses dans les fichiers du    code source, c'est pourquoi elles sont qualifiées d'extrinsèques. Elles peuvent être ajoutées par des opérateurs extérieurs : des services éditoriaux, des agrégateurs, [des catalogues](https://zbmath.org/software/), etc. Elles peuvent être aussi fournies par la personne qui a écrit le code source, comme c'est le cas avec l'archive ouverte HAL. [Les informations à compléter sont indiquées dans un formulaire de saisie.](http://doc.hal.science/deposer/deposer-le-code-source/#)
...
{% include figure.html filename="exercice_codemeta.gif" caption="Figure 11 : Créer un fichier CodeMeta." %}
...
{% include figure.html filename="exercice_codemeta_gitlab.gif" caption="Figure 12 : Ajouter le fichier CodeMeta à votre dépôt." %}
...
* Pour toute demande relative à Software Heritage, utiliser [le formulaire de contact](https://www.softwareheritage.org/contact/). 
* Voir aussi [la Foire Aux Questions](https://www.softwareheritage.org/faq/). 
* Vous pouvez suivre l'actualité de Software Heritage [sur le blog](https://www.softwareheritage.org/blog/) ou [*via* la newsletter](https://www.softwareheritage.org/newsletter/).
