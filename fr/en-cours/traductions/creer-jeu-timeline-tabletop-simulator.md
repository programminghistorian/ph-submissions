...
La leçon présente ensuite *Timeline*, un jeu sur le thème de l'histoire, ainsi que certaines de ses variantes. Les mécaniques de jeu n'étant pas protégées par le droit d'auteur, vous pouvez utiliser celles de *Timeline* pour créer votre propre jeu, que ce soit pour jouer ou pour explorer l'histoire. Une fois ce cadre posé, deux outils numériques couramment utilisés par les personnes qui conçoivent des jeux, qu’elles soient amatrices ou professionnelles, sont présentés : nanDECK et Tabletop Simulator. Les étapes nécessaires à la génération d’un jeu de cartes inspiré de *Timeline* sont ensuite décrites : d'abord en utilisant le modèle de jeu fourni et consacré à l’histoire de l’île de La Réunion - France ; puis avec les éléments d’un prototype de deck de 6 cartes que les lecteurs et lectrices peuvent télécharger et imprimer eux-mêmes.
...
Cela étant, il se peut que vous soyez intéressé, pour la création de prototypes de cartes, par nanDECK, un outil couramment utilisé par les personnes qui créent des jeux, qu’elles soient amatrices ou professionnelles — c’est le cas notamment d’[Elizabeth Hargrave](https://fr.wikipedia.org/wiki/Elizabeth_Hargrave), une autrice de jeux de société.[^15]
...
#### macOS
nanDECK peut être téléchargé et exécuté sur macOS grâce à l'émulateur Windows [Wine](https://www.winehq.org/). Cependant, les personnes utilisant des ordinateurs Apple fonctionnant sous macOS Catalina ou versions ultérieures devront effectuer des démarches supplémentaires, puisque Wine n’est plus compatible avec les systèmes ayant abandonné la prise en charge des applications 32 bits.
