...
Le manuel officiel de nanDECK[^16] propose une solution de contournement qui consiste à [installer Winebottler](https://winebottler.kronenberg.org/) et [XQuartz](https://www.xquartz.org/) afin de faire fonctionner la version Windows sur un ordinateur MacOS. Les personnes concernées peuvent également envisager d'installer et d'utiliser nanDECK via un émulateur tel que [Virtual Box](https://www.virtualbox.org/) ou recourir à une application payante telle que [Parallels](https://www.parallels.com/products/desktop/).
