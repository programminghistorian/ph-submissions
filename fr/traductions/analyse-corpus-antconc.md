...
## Introduction
L'analyse de corpus est un type d'analyse textuelle qui permet de faire des comparaisons d'objets textuels à grande échelle — la fameuse "lecture à distance" (_distant reading_). Cela nous permet de voir des choses que nous n'observons pas nécessairement lorsque nous lisons à l'oeil nu. Si vous avez une collection de documents, vous voudrez peut-être trouver des schémas grammaticaux ou les phrases récurentes dans votre corpus. Vous voudrez peut-être aussi identifier de manière statistique les expressions fréquentes ou peu fréquentes chez un-e auteur-e ou dans un genre particulier, trouver des exemples spécifiques de structures grammaticales ou encore examiner beaucoup d'occurrences d'un concept particulier à travers une série de documents. L'analyse de corpus est surtout utile pour vérifier des intuitions et/ou trianguler des résultats issus d'autres méthodes numériques. 
...
D'autres options gratuites pour les éditeurs de texte, telles que [Notepad++][53] (Windows) ou [TextWrangler][54] (Mac), offrent des fonctionnalités plus avancées, et sont particulièrement utiles pour le nettoyage de texte. Par nettoyage de texte, j'entends la suppression des informations extratextuelles qui apparaissent régulièrement tout au long du texte. Si vous conservez cette information, elle fera partie de vos données. Ainsi, les logiciels d'analyse de texte traiteront ces mots dans le décompte de mots, les analyses statistiques et les relations lexicales. Par exemple, vous pouvez supprimer les en-têtes et pieds de page standards qui apparaîtront sur chaque page. Voir ["Nettoyer ses données avec OpenRefine"](https://programminghistorian.org/fr/lecons/nettoyer-ses-donnees-avec-openrefine) pour plus d'informations sur la façon d'automatiser cette tâche. Sur des corpus plus petits, il peut être plus facile de le faire vous-même, et vous aurez une bien meilleure idée de l'allure de votre corpus de cette façon.

Enregistrez l'article en tant que fichier .txt sur le bureau. Vous pouvez faire un petit nettoyage de texte, par exemple efface le titre ou l'auteur-e (supprimez-les, puis enregistrez de nouveau le fichier). Rappelez-vous que tout ce que vous laissez dans le fichier peut et sera traité comme du texte par un logiciel d'analyse de texte.
...
La construction de corpus est un sous-domaine à part entière. Voir Douglas Biber, « [_Representativeness in Corpus Design_](http://llc.oxfordjournals.org/content/8/4/243.abstract) », _Literary and Linguistic Computing_, 1993, 8 (4) : 243-257 et Martin Wynne (dir.) [_Developing Linguistic Corpora : a Guide to Good Practice_](http://www.amazon.com/Developing-Linguistic-Corpora-Practice-Guides/dp/1842172050/ref=sr_1_1_1) (2005) pour plus d'informations. 
...
**_Concordance_ :** Cela vous montrera ce que l'on appelle la vue mot-clé en contexte (_KeyWord In Context_, abréviation KWIC, plus d'informations à ce sujet dans une minute), en utilisant la barre de recherche en dessous.  
...
L'une des forces des outils de corpus comme AntConc, c'est de trouver des schémas de langage que nous avons du mal à identifier en tant que simples lecteurs et lectrices. Les petits mots ennuyeux comme *le, la, les, je, il, elle, un, une, avoir, être, faire* (*the, I, he, he, she, a, a, an, is, have, will*) sont particulièrement difficiles à suivre en tant que lecteur-trice, parce qu'ils sont si communs, mais les ordinateurs sont très doués pour les examiner. Ces mots sont communément connus sous le nom de "mots vides" (_stopwords_) en humanités numériques ; ce sont souvent des marques caractéristiques d'un-e auteur-e ou d'un genre. Par conséquent, ils peuvent être des termes de recherche très puissants en eux-mêmes ou combinés à des termes plus axés sur le contenu, ce qui aide les chercheurs et chercheuses à identifier des tendances dont ils et elles n'étaient peut-être pas conscient-e-s. 


Dans le champ de recherche en bas, tapez "_the_" et cliquez sur "Démarrer". La vue Concordance vous montrera chaque fois que le mot apparaît dans notre corpus de critiques de films, et les mots qui entourent ces occurrences. C'est ce qu'on appelle une visionneuse "mots-clés en contexte" (_Key Words in Context_). 

{% include figure.html filename="the-thinking.png" caption="'_The_' est un mot commun." %}


(14618 fois, selon la case "_Concordance Hits_" en bas au centre.) 
...
Essayez une recherche similaire pour "_a_". Les deux mots "_a_" et "_the_" sont des articles, mais l'un est un article défini et l'autre un article indéfini - et les résultats que vous obtiendrez vous en donneront une idée.


Maintenant que vous êtes à l'aise avec une ligne KWIC, recommencez avec "_shot_" : vous obtiendrez des exemples à la fois du nom ("_line up the **shot**_") et du verbe ("_this scene was **shot** carefully_").
...
Pour obtenir la liste complète des opérateurs génériques disponibles et ce qu'ils signifient, allez à _Global Settings_ > _Wildcard Settings_ 
...
De plus, dans les corpus en anglais, il se peut qu'une seule lettre *s* apparaisse, au sommet de la liste des collocations. Elle représente les *'s* possessifs (l'apostrophe ne sera pas comptée) - mais AntConc considère qu'il s'agit d'un autre mot. Un autre exemple de ceci est le *t* apparaissant avec *don*, dans le cas de la contradction de *do not* en *don't*. Comme ils apparaissent si souvent ensemble, il est fort probable qu'ils soient colocalisés. 
...
Réfléchissez bien à ce dont pourrait ressembler un corpus de référence pour votre propre recherche (par exemple, un corpus de référence constitué de tous les romans d'Agatha Christie serait très utile afin de comparer un corpus d'analyse composé de ses écrits dans ses dernières années). Rappelez-vous, encore une fois, que la construction de corpus est un sous-domaine à part entière.


Allez dans _Settings_ > _Tool preferences_ > _Keyword List_  
Sous "_Reference Corpus_" assurez vous que "_Use raw files_" est coché
Puis, cliquez sur _Add Directory_ > _Open_ pour ouvrir le répertoire contenant les fichiers qui composent le corpus de référence
...
Et bien sûr, vous pouvez retourner ces questions pour répondre à d'autres questions de recherche :
...
* Pourquoi comparer deux corpus ? 
* Quels sont les types de requêtes qui rendent les questions de recherche pertinentes ?
* Aux principes de construction de corpus : à l’échantillonnage et à la représentativité du corpus. 
...
[Note du traducteur]: 
En France, des outils similaires à AntConc ont dévéloppées par la textométrie et la logométrie, souvent par des historien-ne-s. On peut nommer notamment [Hyperbase](http://ancilla.unice.fr/), [Iramuteq](http://iramuteq.org/), [Lexico](http://www.lexi-co.com/) ou [TXM](http://textometrie.ens-lyon.fr/?lang=fr). On peut aussi consulter l'ouvrage de Ludovic Lebart et André Salem, [Statistique textuelle](http://lexicometrica.univ-paris3.fr/livre/st94/st94-tdm.html), 1994. 
