...
Par défaut, la première cellule d'un nouveau carnet Jupyter est toujours une cellule de code. Voici comment transformer celle-ci en cellule Markdown. Au haut de l'interface Jupyter Notebook se trouve une barre d'outils dont les fonctions s'appliquent à la cellule courante. L'un des outils est une liste déroulante dont l'option par défaut est &laquo; Code &raquo;. Cliquez sur cette liste déroulante et choisissez plutôt "Markdown". (Vous pouvez aussi utiliser le raccourci clavier *Esc+m* pour désigner la cellule courante comme une cellule Markdown, ou encore *Esc+y* pour la désigner à nouveau comme une cellule de code.)
