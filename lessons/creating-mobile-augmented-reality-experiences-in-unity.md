...
{% include figure.html filename="ar-dev-2.png" caption="With HP Reveal 3.6, you can overlay multimedia content such as videos over the pages of a book." %}
...
In addition to the above software requirements, you will also need to make sure that your laptop or desktop computer aligns with [the minimum hardware requirements for Unity development.](https://unity3d.com/unity/system-requirements)

### Unity

Since the release of Unity 2017.2, the Vuforia SDK is integrated into the Unity Editor. The [Vuforia SDK](https://developer.vuforia.com/) is a suite of assets and code snippets that work alonside Unity to make it easier to quickly develop and build AR content for a variety of platforms. If you are unable to download Unity 2017.2 or later, [consult this archived lesson for earlier versions of Unity](link to previous lesson). To download Unity and Vuforia, go to the [Unity website](https://store.unity.com/download) and download the latest version of the Unity installer. In the Choose Components dialog box, select Vuforia Augmented Reality Support in addition to either Android Build Support and/or iOS Build Support, depending on your target mobile device platform. Once the download completes, start Unity and follow the setup prompts. If Unity asks if you are creating a personal or professional account, choose personal account.

### Java Development Kit

Download and install the [Java Development Kit 8](http://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html) for your operating system.
...
Download and install [Android Studio](https://developer.android.com/studio/index.html). Android Studio is the official IDE (integrated development environment) for creating Android applications. However, keep in mind that Android Studio is a very large program that will take some time to download. 

1. Start the SDK manager after installation.  Install any packages that are automatically selected, but make sure that the Android SDK Platform-tools and Android SDK Build-tools options are both selected. 
2. Scroll down to the folder labelled Extras and select the Android Support Repository and Google USB Driver. 
...
2. A Mac running OS X 10.10.5 or later. If you meet these technical requirements, download [Xcode 9](https://itunes.apple.com/us/app/xcode/id497799835?ls=1&mt=12) from the Apple app store. 
...
Unity should automatically detect the location of the Java Development Kit once it has finished installing. If it does not,
...
2. Add an Image Target to your scene by selecting "Vuforia > Image" in the GameObject dropdown menu. 
...
Go to the [Vuforia developer portal](https://developer.vuforia.com/).
...
5. To access your license key, return to the "License Manager" page in the Vuforia developer portal and click on the application name you just created. Copy the alphanumeric string on the right.
6. Return to Unity and File > Build Setting sand click on the "Player Settings" button in the bottom of the pop up window. 
7. Navigate back to the Inspector panel and click on the "XR Settings" option located at the bottom of the accordion menu. Select "Vuforia Augmented Reality Support." 
8. Select ARCamera in the Hierarchy pane. In the inspector panel, click on the Open Vuforia Configuration button in the Vuforia Behavious (Script)" component. Copy and past your license key in the App License Key text field.
...
1. Go to the [Vuforia developer portal](https://developer.vuforia.com/) and click on "Develop" in the top menu and then select Target Manager."
2. Click on the Add Database button on the Target Manager page. In the dialog box that appears, give your database a name and select the Device option. Click Create.
4. Once your database has been created, return to the Target Manager page and click on your database. Select Add Target, choose Single Image as the type, and upload your trigger image by clicking on the Browse button.
...
Vuforia assigns target images a feature tracking rating on a scale of 1 to 5 stars. Navigate to the Image Targets database in the Vuforia Developer Portal and select the image you just uploaded. Click on the Show Features link to reveal the image's trackable features.
...
2. Select the author image in your hierarchy panel. You can now move your author image in the x, y, or z axis by clicking on dragging on the red, blue, or green arrows. 
...
1. Select the author image in the hierarchy panel and navigate to its "Transform" component in the inspector panel. Click on the small cog icon in the upper right corner of the "Transform" component and select "Reset."
...
Go to the search bar within your project hierarchy panel and search for "DefaultTrackableEventHandler" C# script. Click to open in Monodevelop. To create a public game object to store the "finder" game object you just created, navigate to the section of the script labelled "#region PROTECTED_MEMBER_VARIABLES" and write the following script and save your changes:
...
Return to Unity and click and drag the "finder" game object into the public variable slot now available in your ImageTarget's "Video Trackable Event Handler" component in the inspector panel. Within your C# script, press control (or command) "F" and search for "protected virtual void OnTrackingFound()." At the end of the "OnTrackingFound" method, add the following script:
...
These lines of code tell the application to disable the finder game object if the target image is found and to enable the finder game object if it is lost.
...
2. Select Add Current Add Open Scenes.
3. Click on Player Settings and navigate to the inspector panel. 
...
1. Open the device manager on your computer and right click on the Android phone attached as a device. Select "Update Driver Software."
2. Open the Android SDK manager and ensure that the "Google USB Driver" is installed.
...
1. Click on "Player Settings..." and expand the option labelled "Other Settings." 
2. Add a description in the text field labelled "Camera Usage Description" (e.g. "This app requires access to your device camera."). 
3. Next, set the "Target Minimum iOS Version" to 9.0. 
...
8. Switch over to the "Unity-iPhone" targets menu and click on the tab labelled "General." Make sure that "Automatically manage signing" is selected and switch your Team to the Apple ID account you just added. 
