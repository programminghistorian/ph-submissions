...
This lesson is intended for those whose research or interest has led them to look at public-domain digitized books on HathiTrust (HT) or Internet Archive (IA), two of the largest digital libraries in the world. Specifically, the lesson will equip users who want to know more about visual layout (*mise en page*) and illustration. For instance, my own research is on changes in the frequency and type of pictures in early nineteenth-century children's books. This means I count how many pictures per book and whether they are made by relief or intaglio printing processes. Another use case might be comparing changes in illustration among editions of the same book. There are countless further research questions concerning the characteristics of the extracted pictures: color, size, number of figures, etc.
...
Now we can use `conda` to install our first two packages: a localized version of `pip` and Jupyter Notebooks. All the other required packages (gzip, json, os, sys, and time) are part of the [standard Python library](https://docs.python.org/3/library/).
...
This will run the notebook server in your shell and launch your default browser with the Jupyter homepage. This homepage will show all the files in the current working directory. Click on both the `hathitrust.ipynb` and `internetarchive.ipynb` notebooks to open them in new browser tabs. From now on, we don't need to run any commands in the shell. The notebooks allow us to execute Python code and have full access to the computer's filesystem.
...
In the HT notebook, examine the very first cell. Fill in your API tokens as directed. Then run the cell by clicking Run. 
...
In the first cell of the IA notebook, enter your sign-in email and password as directed. Run the cell.
