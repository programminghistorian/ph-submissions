...
We expect the course to take two or three hours to complete. If you've heard of linked open data before you might complete it more quickly. If this is all new you might find that it's rather abstract the first time round. Really the best way to learn about linked data is to mess around with some!
...
We can now search for person `15601` and, providing the data is correctly marked up, know that we are getting the same person - in this case, the film star - every time.
...
The next step is to have a way of *describing* the relationship of Anne Hathaway, our `15601`, to something else. Suppose we have a film that Anne Hathaway acted in. How do we indicate that? In linked data this is done using a 'triple'. Let's make one up now:
...
Hold on a minute! There's more than one film called *One Day*. Let's fix that with another arbitrary number: `873823`.
...
But you have probably used, or been part of, lots of linked data products and resources. The BBC's coverage of the London Olympics in 2012 was built upon linked data principles. As [this blog post]() makes clear, the linked data part was so that they could connect up all of the BBC's information about the Olympics. With coverage of something as complex as the Olympic Games by a major news organisation, you can see that there has to be some way to manage the data.
...
Linked data is also used in lots of academic projects. It's good for modelling messy data in a non-hierarchical way. Here's a good example: medieval motets were mash-ups of sacred and secular texts and music. Mapping all the connections between them would look something like a tube map. Sound familiar? You can read about the rationale for using linked open data for the job [here.]()

A final example you have almost certainly seen is on Wikipedia. The entry for [Seinfeld]() has a box (it appears on the right-hand side of my screen). This is structured data. Because of its structure it can be extracted and made into triples. Those triples can be found, and queried, here: [http://wiki.dbpedia.org/]() We'll be looking at DBpedia, and how we can use it to find information *within Wikipedia* that you cannot find just by searching Wikipedia. This really is linked data in action.
...
That means we're all talking about the same thing, and only that thing. It's more complicated than it sounds; in real life we do this all the time without worrying about it.

Back in section 1 of this course we used two different numbers to identify our two different Anne Hathaways.
...
That looks a lot like a URL but it's a URI. To see that, try pasting it into a web browser and seeing if it takes your browser to a page. It doesn't because, although it is structured like a URL it isn't one. To put it simply, a URL is a URI but a URI is not necessarily a URL. URLs are subsets of URIs. A URI can describe anything, anything at all; a URL describes the location of something on the web.

Why the confusing state of affairs?

There are many websites round the world with pages called things like `/home` or `/faq`. But there is no confusions because the first part of the URL makes all of these pages unique. In the address `http://www.bbc.co.uk/faqs` it is the `bbc.co.uk` part which makes the subsequent pages unique. This is called the domain.

This is so obvious to people who use the web all the time that they don't think about it. You probably also know that if you want to start a website called `bbc.co.uk` you can't, because that name has already been registered with the appropriate authority, which is the Domain Name System. The registration guarantees uniqueness. This naturally lends itself to creation of URIs, which also have to be unique.

I don't own the domain `history.ac.uk` but I asked the webmaster if it was OK for me to use the domain + `tobias-project` to create some URIs. I know that no one else should ever be creating URIs with this same form, so I can be confident that they are unique.

The key point here is that most URIs you see will take a form that looks very much like a URL. Don't worry too much about it.

However it is possible to construct a URI differently. We have all kinds of ways of uniquely identifying people and things and, again, we rarely think or worry about it. Think of barcodes, National Insurance Numbers or even your postal address; in the developing world mobile phone numbers are frequently put up as shop signs precisely because they are unique. All of these could be used as URIs.

So a URL tells you the location of a web page or a file or something similar. A URI just does the job of identifying something. Just as a barcode 9-788433-910400 uniquely identifies a paperback edition of **Estrella distante** by Roberto Bolano but doesn't tell you where to get a copy; for that you would need something like a library shelfmark.

There is a little bit of jargon around URIs: people talk about whether they are, or are not, *dereferenceable*. That just means \*can it be turned from an abstract reference into something else. For example, if you paste a URI into the address bar of a browser, will it return something? For example a URI for Jeff Koons is:

    http://viaf.org/viaf/5035739

If you put that into the browser you will get back a web page about Jeff Koons. This is very handy - for one thing, it's not obvious from the URI who or even what this is. Similarly, if we treated a mobile phone number (with international code) as URI for a person then it should be derefrenceable: someone might answer, and it might even be Jeff Koons.

But this is not essential. Lots of URIs are not dereferenceable. Don't worry too much about this term, but if you see it you'll know what it means.
...
We don't want to type this out every time we refer to this person (Anne Hathaway, you'll remember). So at the top of the file we'll announce our shortcut:

    @prefix toby: <http://data.history.ac.uk/tobias-project/persons>

Then Anne is simply `toby:15601`, which is easier on the eye.

Let's now move from Anne Hathaway to William Shakespeare and use Turtle to encode some stuff about his works. We'll use Dublin Core, a library metadata standard, as one of our prefixes, the Library of Congress Control Number authority file for another, and the last one should be familiar to you:
...
Turtle also allows you to combine triples without bothering to repeat each URI when you've only just used it. Let's add the date when scholars think Macbeth was written:
...
You can use a comma if the subject is the same but the predicate and object are different, or a semi-colon if the subject and predicate are the same and only the object is different.
...
What's a SKOS concept? That brings us onto another area where RDF let's us choose something important: the way in which we describe things. You can make up your own, which is what we've done from time to time in this course, but it's usually more sensible to use one that has already been written. Clearly if you are describing marine fauna you would want to use a different classification system than if you were describing Etruscan tombs.

Here we are using SKOS (Simple Knowledge Organization System), which is designed for encoding something like a thesaurus or a taxonomy, which is convenient because this is from a project to publish a thesaurus of British and Irish History:
...
If you're familiar with XML this will be like mother's milk to you. If you're not you might prefer a format like Turtle. But the advantage here is that in creating my RDF/XML I can use the usual tools available with XML, like dedicated XML editors and parsers, to check that my RDF/XML is correct.

Whatever you are looking to represent with linked open data, we recommend that you find an existing ontology and use it, rather than try to write your own. Have a look at the main page here for a list of some of the most popular vocabularies:
...
The query language we use for linked data is called SPARQL. It's one of those reflexive acronyms beloved of techie people: *Sparql Protocol and Query Language*.

We're going to run our SPARQL queries on DBpedia, which is a huge linked data set derived from Wikipedia. As well as being full of information that is very difficult to find through usual Wikipedia interface, it has several SPARQL "end points" - interfaces where you can type in SPARQL queries and get results from a data set.

The one I use is called snorql: [http://dbpedia.org/snorql/]() These end points occasionally seem to go offline, so if you can find this one try searching for *dbpedia sparql* and you should find something similar.
...
If you've ever written a database query in the query language SQL, this will look pretty familiar and it will certainly help you to learn SPARQL. If not, don't worry: you'll pick it up.
...
Hit 'go' and, if you left the drop-down box as 'browse' you should get two columns labelled "b" and "c". So what just happened? And how did I know what to type?

I didn't, really, and that is one of the issues with SPARQL end points. You have to try things and find out what terms are used. Because this comes from Wikipedia, and I was interested in what information on historians I could find, I went to the Wikipedia page for Tony Judt:

[https://en.wikipedia.org/wiki/Tony\_Judt]()

The final bit after the slash is a very good bet: they're are likely to be triples about Tony Judt, especially because this is a fairly large article. The ?a and the ?b are just place-holders: I could equally well have typed ?whatever and ?you\_like and the columns would have had those headings. When you want to be more precise about what you are returning, it will be more important to label columns meaningfully.

Try your own SPARQL query now: choose a Wikipedia page and copy the end part of the URL, after the final slash, and put it in place of Tony\_Judt. Then hit 'go'.
...
I changed *Tony\_Judt* to *London* and ran the query again. Try that now and have a look.

Towards the bottom of the columns produced I can see Mayor\_of\_London. If you run the query and click on *Mayor\_of\_London* you will see that a new SPARQL query is generated for you. This is very helpful. From the query I can see that there is the term *dbpedia2:title* and the string *:Mayor\_of\_London* (the colon with nothing before it means it's locally declared, in other words this is just a string that DBpedia happens to use. Let's put all of this into a query:
...
This gives me four results, including two people: Ken Livingstone and Boris Johnson. But wait, at the time of writing Sadiq Khan has been Mayor of London for some months. Why isn't he in this list?

This brings us to the problem with Dbpedia that I mentioned earlier: it's not very consistently marked up with structural information of the type DBpedia uses (remember the differences in the info boxes for Hemingway and Twain?) By the time you run the query, maybe Sadiq Khan will appear in the results, but for now it's a warning to us not to treat results as a faithful reflection of real life: they're a faithful reflection of what is in the dataset, and as I type Sadiq Khan is not listed as having the title Mayor of London in the triples.

Let's finish this section, and the course, with a slightly more complicated example. Go back to the Tony Judt query and run it again.

    SELECT * WHERE {
    :Tony_Judt ?b ?c
    }

Go down the second column of results and you will see `:Category:British_historians`. Click on it. If you scroll down you will start to see a bit list of British Historians. This is useful but let's combine our query with something else.

First, let's make sure that we understand the British Historians query by simplifying it
...
The change I've made here is that instead of `SELECT *` I'm now saying `SELECT ?name`. This means *just return the column that I'm calling ?name* (I could have called it something else).

The nice thing is that by repeating ?name in subsequent lines of the query. So if I want to find British historians born in London I add a line to filter the first query:
...
If I now want to restrict the list to women I need to know how to do that, so I'll click on the link for Joan\_Thirsk, where I can see that she's in in the category British\_women\_academics. Let's add another line to our query:
...
That's good, but, again, shows the limitations of the underlying data: I'm not getting Mary\_Dorothy\_George or Alison\_Weir, who were in the previous list of British historians born in London, because they are not marked up as British women academics!
...
SELECT \* WHERE { ?a <http://dbpedia.org/ontology/openingTheme> <http://dbpedia.org/resource/Tom_Waits> . }
