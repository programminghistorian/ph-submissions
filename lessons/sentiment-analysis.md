...
In the following decade, hundreds of new studies sprouted up from the e-mails pursuing questions as diverse as [social network theory](https://en.wikipedia.org/wiki/Social_network), community and [anomaly detection](https://en.wikipedia.org/wiki/Anomaly_detection), gender and communication within organizations, behavioral change during an organizational crisis, and insularity and community formation.
...
First, we have to tell Python where the NLTK functions are located. At the top of our file, we will import *VADER* function and assign it to the variable "*sentimentID*" for easy access (Python programs will often reassign the name of functions from other libraries to a simpler function for ease of access).
