...
É depois possível identificar cada coluna pelo nome, por exemplo a coluna que indica a data de nascimento é identificada assim:
...
```rownames=4``` 
em que o algarismo indica o número da coluna
...
Podemos assim identificar as escolas (literárias) presentes no material (removemos da figura os casos em que a escola é desconhecida, marcados por "desc"):
...
O que nos permite por exemplo fazer um diagrama de caixa pela variedade do português, e não pelos autores. Escolhi observar o uso das vírgulas.
